...
impl ExternalUser {
    ...
    pub fn account(&self, runtime: &mut StandaloneRuntime) -> Account {
        runtime
    ...
    pub fn pool_deposit(&self, runtime: &mut StandaloneRuntime, amount: Balance) -> TxResult {
        let tx = self
            .new_tx(runtime, POOL_ACCOUNT_ID.into())
    ...
    pub fn pool_ping(&self, runtime: &mut StandaloneRuntime) -> TxResult {
        let tx = self
            .new_tx(runtime, POOL_ACCOUNT_ID.into())
    ...
    pub fn pool_stake(&self, runtime: &mut StandaloneRuntime, amount: u128) -> TxResult {
    ...
    pub fn pool_unstake(&self, runtime: &mut StandaloneRuntime, amount: u128) -> TxResult {
        let args = json!({ "amount": format!("{}", amount) })
            .to_string()
            .as_bytes()
            .to_vec();
    ...
    pub fn pool_withdraw(&self, runtime: &mut StandaloneRuntime, amount: u128) -> TxResult {
        let args = json!({ "amount": format!("{}", amount) })
            .to_string()
            .as_bytes()
            .to_vec();
    ...
    pub fn pool_pause(&self, runtime: &mut StandaloneRuntime) -> TxResult {
        let tx = self
            .new_tx(runtime, POOL_ACCOUNT_ID.into())
    ...
    pub fn pool_resume(&self, runtime: &mut StandaloneRuntime) -> TxResult {
        let tx = self
            .new_tx(runtime, POOL_ACCOUNT_ID.into())
    ...
    #[allow(dead_code)]
    pub fn pool_vote(&self, runtime: &mut StandaloneRuntime, amount: u128) -> TxResult {
        let args = json!({ "amount": format!("{}", amount) })
            .to_string()
            .as_bytes()
            .to_vec();
    ...
    #[allow(dead_code)]
    pub fn get_account_staked_balance(&self, runtime: &StandaloneRuntime) -> Balance {
        let balance = runtime
            .view_method_call(
    ...
    pub fn get_account_unstaked_balance(&self, runtime: &StandaloneRuntime) -> Balance {
        let balance = runtime
            .view_method_call(
                &POOL_ACCOUNT_ID.into(),
...
pub fn init_pool(initial_transfer: Balance) -> (StandaloneRuntime, ExternalUser) {
...
pub fn is_pool_paused(runtime: &mut StandaloneRuntime) -> bool {
    call_view(runtime, &POOL_ACCOUNT_ID.into(), "is_staking_paused", "{}")
}

pub fn reward_pool(runtime: &mut StandaloneRuntime, amount: Balance) {
    let mut pool_account = runtime.view_account(&POOL_ACCOUNT_ID.into()).unwrap();
...
pub fn pool_account(runtime: &mut StandaloneRuntime) -> Account {
