...
impl MultiSigContract {
    ...
    fn execute_request(&mut self, request: MultiSigRequest) -> PromiseOrValue<bool> {
        ...
        for action in request.actions {
            promise = match action {
                MultiSigRequestAction::Transfer { amount } => promise.transfer(UncToken::from_attounc(amount.into())),
                ...
                MultiSigRequestAction::AddKey {
                    ...
                    if let Some(permission) = permission {
                        let allowance = permission
                            .allowance
                            .unwrap_or(DEFAULT_ALLOWANCE.into())
                            .into();
                        promise.add_access_key_allowance(
                            public_key.into(),
                            Allowance::limited(UncToken::from_attounc(allowance)).unwrap_or(Allowance::Unlimited),
                ...
                MultiSigRequestAction::FunctionCall {
                    ...
                    method_name,
                    args.into(),
                    UncToken::from_attounc(deposit.into()),
                    Gas::from_gas(gas.0),
                ),
    ...
    ********************************/
    ...
    fn current_member(&self) -> Option<MultisigMember> {
        ...
        let members: Vec<MultisigMember> = self.members.iter().cloned().collect();
    ...
    fn add_member(&mut self, promise: Promise, member: MultisigMember) -> Promise {
        ...
        match member {
            MultisigMember::AccessKey { public_key } => promise.add_access_key_allowance(
                public_key.into(),
                Allowance::limited(UncToken::from_attounc(DEFAULT_ALLOWANCE)).unwrap_or(Allowance::Unlimited),
    ...
    fn delete_member(&mut self, promise: Promise, member: MultisigMember) -> Promise {
        ...
        let request_ids: Vec<u32> = self
            .requests
            .iter()
            .filter_map(|(k, r)| if r.member == member { Some(k.to_owned()) } else { None })
            .collect();
    ...
    fn remove_request(&mut self, request_id: RequestId) -> MultiSigRequest {
        ...
        let mut num_requests = self
            .num_requests_pk
            .get(original_member.to_string().as_str())
            .unwrap_or(&0)
            .to_owned();
...
mod tests {
    ...
    use unc_sdk::{testing_env, PublicKey};
    use unc_sdk::{AccountId, VMContext};
    ...
    pub fn bob() -> AccountId {
        AccountId::try_from("bob".to_string()).unwrap()
    }
    ...
    fn context_with_key(key: PublicKey, amount: UncToken) -> VMContext {
        context_with_account_key(alice(), key, amount)
    }

    fn context_with_account(account_id: AccountId, amount: UncToken) -> VMContext {
    ...
    fn context_with_key_future(key: PublicKey, amount: UncToken) -> VMContext {
        VMContextBuilder::new()
            .current_account_id(alice())
