...
fn register_len(register_id: u64) -> Option<u64> {
    let len = unsafe { unc_sys::register_len(register_id) };
    if len == core::u64::MAX {
        None
    } else {
        Some(len)
...
fn storage_remove(key: &[u8]) {
    unsafe { unc_sys::storage_remove(key.len() as _, key.as_ptr() as _, EVICTED_REGISTER) };
}

fn input() -> Option<Vec<u8>> {
    unsafe { unc_sys::input(ATOMIC_OP_REGISTER) };
    let len = register_len(ATOMIC_OP_REGISTER)?;

    let buffer = vec![0u8; len as usize];

    // Read data from register into buffer
    unsafe { unc_sys::read_register(ATOMIC_OP_REGISTER, buffer.as_ptr() as _) };
...
mod tests {
    ...
    use utility_workspaces::{Contract, Worker};
    ...
    async fn prop(
        ...
        let state_items = contract.view_state().await?;
        for (k, v) in bytes {
