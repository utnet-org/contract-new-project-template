...
use unc_sdk::{borsh, env, unc, AccountId};
...
pub struct LockupInformation {
    ...
    /// are liuncly released from the moment tokens are unlocked, defined by:
...
impl VestingScheduleWithSalt {
    pub fn hash(&self) -> Hash {
        env::sha256(&borsh::to_vec(&self).expect("Failed to serialize"))
    }
}
