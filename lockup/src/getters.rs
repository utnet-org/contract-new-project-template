...
impl LockupContract {
    ...
    pub fn get_owners_balance(&self) -> WrappedBalance {
        (env::account_balance().saturating_add(UncToken::from_attounc(self.get_known_deposited_balance().0)))
            .saturating_sub(UncToken::from_attounc(self.get_locked_amount().0))
            .as_attounc()
            .into()
    }
